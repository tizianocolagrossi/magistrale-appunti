...
Inject some query into another query with input not checked for safety reason. To check if there is some SQLinjection we can use special character such as ' # " to see if the website retur us an error. Then we can manipulate the input to change the final query and to make own custom query. We can now use INFORMATION_SCHEMA tables to map the database. One tool to do some SQLinjection is 'sqlmap'